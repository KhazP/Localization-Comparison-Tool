...
class PropertiesParser extends LocalizationParser {
  ...
  Future<Map<String, String>> parse(
    ...
    for (String line in lines) {
      ...
      if (trimmedLine.endsWith('\\')) {
        ...
        currentLineBuffer +=
            line; // Add the non-trimmed line if it's the end of a logical line
      }

      String logicalLine = currentLineBuffer
          .trimLeft(); // Trim only left for processing, right trim later
      ...
      List<String> possibleSeparators = [
        '=',
        ':'
      ]; // Whitespace as separator is more complex
      ...
      if (separatorIndex == -1) {
        separatorIndex =
            logicalLine.indexOf(RegExp(r'\\s+')); // First whitespace sequence
        if (separatorIndex != -1 &&
            logicalLine
      ...
      if (separatorIndex != -1) {
        ...
        String value = logicalLine
            .substring(separatorIndex + 1)
            .trimLeft(); // Only trim left of value initially
        ...
        if (key.isNotEmpty) {
          if (translations.containsKey(key)) {
            debugPrint(
                'Warning: Duplicate key "$key" found in .properties file ${file.path}. Overwriting.');
          }
        ...
        if (key.isNotEmpty) {
          if (translations.containsKey(key)) {
            debugPrint(
                'Warning: Duplicate key "$key" (no separator) found in .properties file ${file.path}. Overwriting with empty value.');
          }
          translations[key] = '';
          debugPrint(
              'Info: Line without separator in ${file.path} treated as key "$key" with empty value: "$logicalLine"');
        }
      }
    }
    if (translations.isEmpty &&
        lines.any((l) =>
            ...
            !l.trim().startsWith('!'))) {
      debugPrint(
          'Warning: Parsed .properties file ${file.path} but no key-value pairs extracted (check format).');
    }
  ...
  String _unescapePropertiesString(String str) {
    ...
    str =
        str.replaceAll(r'\\', '\uE000'); // Use a private use area char as temp

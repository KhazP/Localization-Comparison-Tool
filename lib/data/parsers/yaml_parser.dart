...
class YamlParser extends LocalizationParser {
  ...
  Future<Map<String, String>> parse(
    ...
    try {
      ...
      if (yamlData is YamlMap) {
        ...
        debugPrint(
            'Info: YAML file ${file.path} is empty or contains only null.');
      } else {
        debugPrint(
            'Warning: YAML content in ${file.path} did not parse to a YamlMap (root type: ${yamlData.runtimeType}). Only root maps are supported.');
      }

      if (translations.isEmpty && yamlData is YamlMap && yamlData.isNotEmpty) {
        debugPrint(
            'Warning: Parsed YAML file ${file.path} but no valid string translations were extracted (check structure).');
      }
    } catch (e) {
  ...
  void _flattenYaml(YamlMap yamlMap, String prefix,
    ...
    yamlMap.nodes.forEach((dynamic keyNode, dynamic valueNode) {
      ...
      final dynamic value =
          valueNode is YamlScalar ? valueNode.value : valueNode;

      if (value is String) {
        translations[newKey] = value;
      } else if (value is YamlMap) {
        // YamlMap from valueNode directly
        _flattenYaml(value, newKey, translations, filePath);
      } else if (valueNode is YamlMap) {
        // Check valueNode if value isn't YamlMap
        _flattenYaml(valueNode, newKey, translations, filePath);
      } else if (value is num || value is bool) {
        translations[newKey] = value.toString();
        debugPrint(
            'Info: YAML value for key "$newKey" in $filePath is num/bool, converted to string.');
      } else if (value == null) {
        translations[newKey] = ''; // Null value becomes empty string
        debugPrint(
            'Info: YAML value for key "$newKey" in $filePath is null, converted to empty string.');
      }
      ...
      // }
        ...
        debugPrint(
            'Warning: Unsupported YAML value type (${value?.runtimeType}) for key "$newKey" in $filePath. Value: $value');
